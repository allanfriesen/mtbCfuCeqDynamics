!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!                                                                         !!!
!!! THIS PROGRAM READS AN INPUT FILE GENERATED BY StochasticSimulation.py.  !!!
!!! THE PROGRAM PERFORMS STOCHASTIC GILLESPIE SIMULATIONS USING ADAPTIVE    !!!
!!! TAU LEAPING AS DESCRIBED BY CAO, GILLESPIE, PETZOLD J CHEM PHYS 2006    !!!
!!!                                                                         !!!
!!! THE PROGRAM IS NOT PARTICULARLY INTENDED FOR GENERAL USE, THOUGH IT IS  !!!
!!! FAIRLY FLEXIBLE FOR AN AD HOC PROGRAM. THE PROGRAM CAN HANDLE           !!!
!!! REACTION NETWORKS INVOLVING FIRST ORDER REACTIONS. WE HAVE NOT          !!!
!!! IMPLEMENTED THE ALGORITHMS FOR TIME STEP SELECTION FOR SECOND AND       !!!
!!! THIRD ORDER REACTIONS, AND THE PROGRAM ASSUMES A PRIORI THAT THE        !!!
!!! PROCESSES ARE FIRST ORDER. PERHAPS IN SOME FUTURE INCARNATION OF THE    !!!
!!! PROGRAM WE WILL EXTEND THE FUNCTIONALITY.                               !!!
!!!                                                                         !!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
iSave = 0
t = 0.d0
remainingSSAsteps = nSSAstep !This code always starts with full SSA, which is natural 
                             !for simulating low infection dose.
...
do while (t < tSim)
    ...
    if( remainingSSAsteps > 0 ) then
        ...
        !If the tau generated from the SSA algorithm passes the end of a simulation stage,
        !then we instead jump to the switch time, and no critical reactions fire.
        ...
        t = t + tauCritical
        if (int(t/dt + dtTolerance) > iSave) then !Including dtTolerance here avoids rare double calls caused by rounding errors
            call saveState
        end if !t + tau > (iSave + 1) * dt
        !loop to randomly select (with approprate probability) which reaction fires next
        tmpPropensitySum = 0.d0
        rand(2) = rand(2) * propensitySum !random number normalized to total reaction propensity
        criticalReaction = -1
        do rxn = 1, nRxn
            tmpPropensitySum = tmpPropensitySum + propensities(rxn)
            if( tmpPropensitySum > rand(2) ) then !keep trying each reaction, until you pass the random value
                criticalReaction = rxn !Once the sum of propensities passes rand(2), we have found the next rxn to fire
                exit
            end if
        end do !
        ...
        remainingSSAsteps = remainingSSAsteps - 1 !finished with this SSA time step
        cycle !back to the top
    else
        tauTentative = tentativeLeap() !generate a provisional tau leap
        ...
        if( tauTentative + t > switchTimes(stage) ) then !also, don't leap past end of stage
            tauTentative = switchTimes(stage) - t + dtTolerance
            updateStage = .true.
        end if
        if( tauTentative > thresholdSSA / propensitySum ) then !if tauTentative is too 
                                                               !long, we risk some populations becoming too small
            call random_number(rand)
            ...
            if( tauTentative < tauCritical ) then
                ...
                !This is done the same way as in the full SSA, except that the calculation
                !includes critical reactions only.
                tmpPropensitySum = 0.d0
            ...
            do rxn = 1, nRxn
                !now, use tau leaping to run the remaining non-critical reactions
            ...
            t = t + tauLeap
            !the boolean updateStage will be .true. if we have reached the start of a simulation stage
            if( updateStage ) then
                !avoid rounding problems by setting t explicitly to the start time of the next stage
                t = switchTimes(stage)
                !update stage index
                stage = stage + 1
                !update rates
            ...
            !if time has progressed at least one save interval beyond the previous save,
            if( int(t/dt+dtTolerance) > iSave ) then
                    !then update trajectories
                    call saveState
            end if !int(t/dt) > iSave
        else
...
!Sometimes SSA steps span past multiple save times; this is likely when all populations are very small.
!To deal with this, we loop over save intervals and write the state more than once if needed.
!This needs to be done *before* populations are updated, when running full SSA.
!If timestep selection is working properly, this issue should not come up when tau leaping is being used.
...
!For efficiency, it is set up to assume only first order kinetics.
!The code could relatively easily be adapted for other situations, 
!but so far, we have not needed this.
...
!This function returns a tentative leap, based on the algorithm given in Cao, Gillespie, Petzold, J Chem Phys 124, 044109 (2006)
...
!This is implemented to return an integer*8. It can of course
!be modified to return a real, if working with the integers is inconvenient
!for some reason
function poidev(xm)
implicit none
real*8 xm,pi,rand
...
data oldm /-1.d0/
if (xm .gt. 1.e6) then
    em = normalDev( xm, sqrt(xm) ) !Gaussian approximation
...
!while loops. This code could be made more compact, but probably there
...
poidev = nint(em,kind=8) !integer*8 type
return
end function poidev
